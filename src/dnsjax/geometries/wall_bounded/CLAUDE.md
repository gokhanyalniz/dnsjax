...
- **Mathematical equivalence**: The IMM is a Schur-complement (capacitance-matrix) reduction. The coupled pressure--velocity system has a 2x2 block structure with interior unknowns (I) and boundary unknowns (B). The 2x2 (Cartesian) or 1x1 (cylindrical) influence matrix M is the Schur complement $S = A_{BB} - A_{BI} A_{II}^{-1} A_{IB}$. The homogeneous data (`p1, p2, v1, v2, q1, q2`) are the columns of $A_{II}^{-1} A_{IB}$; `M_inv` is $S^{-1}$. The correction step (adding $\alpha_i$ times homogeneous solution $i$) is a rank-2 low-rank update -- the same algebraic structure as the Woodbury matrix identity applied to boundary conditions. The bulk-velocity correction is a rank-1 Sherman--Morrison update.
