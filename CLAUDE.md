...
`dnsjax` is a GPU-accelerated pseudo-spectral DNS solver for the 3D incompressible Navier-Stokes equations, written in JAX. It targets triply-periodic flows (Kolmogorov, Waleffe, decaying-box) and wall-bounded flows (plane-Couette). The solver uses a predictor-corrector time integration scheme (Euler + iterative Crank-Nicolson, following Willis 2017 / openpipeflow).
...
geometries/
  ...
  cartesian.py       # Norms with y-integration (Simpson's rule), IMM precomputation, LU solvers
flows/
  monochromatic.py   # Full flow interface for periodic systems (Kolmogorov, Waleffe, decaying-box)
  plane_couette.py   # Wall-bounded flow via influence-matrix method
```
...
**JAX pytree registration**: Flow dataclasses (e.g. `TriplyPeriodicFlow`, `PlaneCouetteFlow`) are registered as JAX pytrees via `register_dataclass_pytree()` in `sharding.py`, allowing them to be passed through `@jit` boundaries as static-like arguments.

**Wall-bounded flows use the influence-matrix method (IMM)**: For plane-Couette, the pressure Poisson equation with preliminary Neumann BCs is solved via LU-factored matrices (`Lk`, `Hk`) precomputed offline in NumPy/SciPy. `params.solver.backend` selects the storage format: `"banded"` (default) keeps the LU factors in LAPACK's packed `ab` layout of shape `(Nkz, Nkx, 3p+1, Ny)` with `p = params.res.fd_order`, and the triangular solves run on device via two `lax.scan`s (see `_banded_solve_device` and `PerModeBandedOperator` in `geometries/cartesian.py`); the `Lk` and `Hk_minus` matvecs are reconstructed on the fly from the shared `D2` / `D1` matrices by `_lk_matvec` / `_hk_minus_matvec` in `flows/plane_couette.py`. `"dense"` keeps the legacy `(Nkz, Nkx, Ny, Ny)` LU factors (plus cached `Lk`, `Hk_minus`) as a reference / rollback path. The IMM constructs homogeneous solutions (`p1`, `p2`) and an influence matrix `M_inv` to find, during timestepping, the correct boundary condition for pressure from the normal derivative of the wall-normal velocity at the wall. Pressure is then solved for with the correct boundary condition, and then the velocity field is calculated with the correct pressure gradient. The IMM matrices are distributed across devices via `jax.make_array_from_callback`.
...
Key sections: `[phys]` (re, forcing, oversampling_factor), `[geo]` (lx, lz, tilt_degree), `[res]` (nx, ny, nz, fd_order, double_precision), `[step]` (dt, implicitness, corrector_tolerance), `[stop]` (max_sim_time, max_wall_time as ISO 8601), `[debug]` (correct_divergence, time_functions, measure_corrections), `[dist]` (np, platform), `[solver]` (backend: `"banded"` or `"dense"`).
