...
geometries/
  ...
  cartesian.py       # Fourier class, norms with Simpson y-integration, CartesianFlow base dataclass, on-device IMM operator assembly and LU factorisation (dense/banded), Kleiser-Schumann IMM iteration, build_cartesian_stepper factory
flows/
...
**Wall-bounded flows use the influence-matrix method (IMM)**: For plane-Couette, the pressure Poisson equation with preliminary Neumann BCs is solved via LU-factored matrices (`Lk`, `Hk`). The entire per-mode setup runs on the device: the FD matrices `D1`/`D2` are the only CPU artefacts and are copied to GPU once, after which `Lk` and `Hk` are assembled and factorised with no further host↔device traffic. All IMM homogeneous data (`p1, p2, v1, v2, q1, q2, M_inv`) is then derived by `CartesianFlow._derive_imm_homogeneous_data` from the already-factored GPU operator. `params.solver.backend` selects the operator-factor storage format: `"banded"` (default) builds `Lk`/`Hk` straight into LAPACK's packed `ab` layout of shape `(Nkz, Nkx, 3p+1, Ny)` with `p = params.res.fd_order` via `_build_Lk_ab_gpu`/`_build_Hk_ab_gpu` and factorises them with the pure-JAX `_banded_lu_factor_batched` scan (LAPACK-compatible partial-pivoted LU; no `(Nkz, Nkx, Ny, Ny)` dense intermediate ever materialises); the triangular solves then run on device via two `lax.scan`s (see `_banded_solve_device` and `PerModeBandedOperator` in `geometries/cartesian.py`), and the `Lk` and `Hk_minus` matvecs are reconstructed on the fly from the shared `D2` / `D1` matrices by `_lk_matvec` / `_hk_minus_matvec`. `"dense"` builds the full `(Nkz, Nkx, Ny, Ny)` matrices (plus cached `Lk`, `Hk_minus`) on the GPU via `_build_Lk_dense_gpu`/`_build_Hk_dense_gpu` and factors them on-device via `DenseJAXSolver` — a reference path kept for parity with the banded backend, and the only path that materialises `(Nkz, Nkx, Ny, Ny)` arrays. The IMM uses the homogeneous solutions (`p1`, `p2`) and influence matrix `M_inv` during timestepping to find the correct pressure boundary condition from the normal derivative of the wall-normal velocity at the wall; pressure is then solved with that BC, and velocity is updated with the corresponding pressure gradient. Operator factors and homogeneous data inherit the kx-sharded layout from the broadcast against `fourier.k2`.
