...
operators.py         # Wavenumber helpers (real_harmonics, complex_harmonics); vector cross product; vmapped 2D/3D phys<->spec FFT wrappers
...
**Wall-bounded flows use the influence-matrix method (IMM)**: For plane-Couette, the pressure Poisson equation with preliminary Neumann BCs is solved via LU-factored matrices (`Lk`, `Hk`) precomputed offline in NumPy/SciPy. `params.solver.backend` selects the storage format: `"banded"` (default) keeps the LU factors in LAPACK's packed `ab` layout of shape `(Nkz, Nkx, 3p+1, Ny)` with `p = params.res.fd_order`, and the triangular solves run on device via two `lax.scan`s (see `_banded_solve_device` and `PerModeBandedOperator` in `geometries/cartesian.py`); the `Lk` and `Hk_minus` matvecs are reconstructed on the fly from the shared `D2` / `D1` matrices by `_lk_matvec` / `_hk_minus_matvec` in `geometries/cartesian.py`. `"dense"` keeps the legacy `(Nkz, Nkx, Ny, Ny)` LU factors (plus cached `Lk`, `Hk_minus`) as a reference / rollback path. The IMM constructs homogeneous solutions (`p1`, `p2`) and an influence matrix `M_inv` to find, during timestepping, the correct boundary condition for pressure from the normal derivative of the wall-normal velocity at the wall. Pressure is then solved for with the correct boundary condition, and then the velocity field is calculated with the correct pressure gradient. The IMM matrices are distributed across devices via `jax.make_array_from_callback`.
