...
| `[init]`   | `start_from_laminar`, `snapshot`, `t0`, `it0`, `isnap0` (initial snapshot counter; inherited on a continuation resume), `force_resume` (continue the resumed trajectory's `t`/`it`/`isnap` even when `phys`/`geo`/`res` changed), `random_field` (in-process random IC; `random_amplitude`/`random_smoothness`/`random_seed`/`random_mean_flow`), `localized_rolls` (in-process deterministic localized-spot IC, wall-bounded only, lower precedence than `random_field`, peak-normalized to amplitude; `localized_rolls_amplitude`/`localized_rolls_width` (physical half-width)/`localized_rolls_wavelength` (cross-roll wavelength)) |
...
- `tests/test_resume.py` covers the snapshot-lineage / resume policy. An **offline unit** (no JAX/mpirun) checks `parameters.trajectory_defining_changes` (identical params → continuation; a `phys`/`geo`/`res` override is reported; `res.double_precision` and non-trajectory sections are ignored). A **subprocess integration** (`mpirun -np 1 -m dnsjax`, plane-couette random IC) checks the IC `state00000.tar` + final snapshot are written with `isnap` metadata and embedded stats (reusing `test_random_smoke._check_run` for clean integration), that an unchanged-param resume **continues** numbering, and that a changed-`re` resume starts a **new trajectory** unless `--init.force_resume True`. Run `--unit-only` to skip the mpirun part.
